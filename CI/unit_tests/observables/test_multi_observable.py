...
class TestMultiSensing:
    ...
    def setup_class(cls):
        ...
        cls.position_observable = PositionObservable(box_length=np.array([1.0, 1.0, 1.0]))
    ...
    def test_compute_observable(self):
        ...
        distance_colloid_1 = np.linalg.norm(colloid_1.pos - self.concentration_observable.source)
        distance_colloid_2 = np.linalg.norm(colloid_2.pos - self.concentration_observable.source)
        distance_colloid_3 = np.linalg.norm(colloid_3.pos - self.concentration_observable.source)

        delta_colloid_1 = distance_colloid_1 - np.linalg.norm(self.colloids[0].pos - self.concentration_observable.source)
        delta_colloid_2 = distance_colloid_2 - np.linalg.norm(self.colloids[1].pos - self.concentration_observable.source)
        delta_colloid_3 = distance_colloid_3 - np.linalg.norm(self.colloids[2].pos - self.concentration_observable.source)
        ...
        director_should_be = [colloid_1.director, colloid_2.director, colloid_3.director]
