...
class TestFindLocation:
    ...
    def test_simple_box(self):
        ...
        """
        bad_reward = self.task(torch.tensor([1, 1, 0]))
        good_reward = self.task(torch.tensor([0, 0, 0]))
        mixed_reward = self.task(torch.tensor([1.0, 0.0, 0]))
    ...
    def test_box_shift(self):
        ...
        good_reward = self.task(torch.tensor([1, 1, 0]))
        bad_reward = self.task(torch.tensor([0, 0, 0]))
        mixed_reward = self.task(torch.tensor([1.0, 0.0, 0]))
    ...
    def test_larger_box(self):
        ...
        bad_reward = self.task(torch.tensor([1000.0, 1000.0, 0.0]))
        good_reward = self.task(torch.tensor([0, 0, 0.0]))
        mixed_reward = self.task(torch.tensor([1000.0, 0.0, 0.0]))
