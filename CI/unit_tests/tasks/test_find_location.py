...
class TestFindLocation:
    ...
    def test_simple_box(self):
        ...
        """
        bad_reward = self.task.compute_reward(np.array([1, 1, 0]))
        good_reward = self.task.compute_reward(np.array([0, 0, 0]))
        mixed_reward = self.task.compute_reward(np.array([1.0, 0.0, 0]))
    ...
    def test_box_shift(self):
        ...
        good_reward = self.task.compute_reward(np.array([1, 1, 0]))
        bad_reward = self.task.compute_reward(np.array([0, 0, 0]))
        mixed_reward = self.task.compute_reward(np.array([1.0, 0.0, 0]))
