...
def test_trajectory_information_exposes_deprecated_killed_status():
    trajectory = TrajectoryInformation(particle_type=0)
    trajectory.terminated.extend([False, True])

    with pytest.warns(DeprecationWarning, match="terminated"):
        assert trajectory.killed is True
