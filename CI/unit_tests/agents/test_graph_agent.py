...
class EspressoTestRLTrainers(ut.TestCase):
    ...
    observable = srl.observables.ColGraph(
        colloids=cols, cutoff=2.0, box_size=np.array([1000, 1000, 1000])
    )

    init_graph = observable.compute_initialization_input(cols)

    # Define the models.
    network = srl.networks.GraphModel(
    ...
    agent = srl.agents.GraphActorCriticAgent(
        particle_type=0,
        network=network,
        task=KillTask(),
