...
def test_cross():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cross_thickness = ureg.Quantity(3, "micrometer")
        center_part = runner.add_cross(
            ureg.Quantity(np.array([25, 25, 0]), "micrometer"),
            ureg.Quantity(40, "micrometer"),
            cross_thickness,
            np.pi / 2.0,
            31,
            ureg.Quantity(10, "newton/ (meter / second)"),  # unused value because fixed
            fric_rot,
            2,
            fixed=True,
        )
        ...
        assert np.linalg.norm(director_after - director_before) > 1e-3
        ...
        np.testing.assert_allclose(
            cutoff,
            coll_rad.m_as("sim_length") + cross_thickness.m_as("sim_length") / 2,
        )
