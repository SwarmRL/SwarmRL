...
from swarmrl.visualization.video_vis import Animations
from swarmrl.visualization.video_vis import load_traj_vis
from swarmrl.visualization.video_vis import load_extra_data_to_visualization
...
class minimum_model(InteractionModel):
    ...
    def calc_action(self, colloids) -> typing.List[Action]:
        ...
        for colloid in colloids:
            ...
            actions.append(
                Action(force=self.act_force * force_mult, torque=np.array([0, 0, self.act_torque* torque_mult]))
            )
...
class extra_data_model(InteractionModel):
    ...
    def calc_action(self, colloids) -> typing.List[Action]:
        ...
        for colloid in colloids:
            ...
            vision_vals = self.vision_handle.compute_single_observable(colloid.id,colloids)
            ...
            # make decisions
            if phase == 0 and self.col_RAM[colloid.id, phase] == self.phase_len[phase]:
                if self.col_RAM[colloid.id,self.n_phases] < schmell_magnitude:  #self.n_phase is the first index for schmellmemory
                    # if the chemics (schmell) get more 
                    self.col_RAM[colloid.id, phase]=0
                    phase=0  # jump to run phase and head straight forward
                    self.col_RAM[colloid.id, phase] = 1
...
class TestFullSim(ut.TestCase):
    ...
    def simulate_model(self, outfolder, mode):
        ...
        rod_params = {
            ...
            "rod_start_angle": 0,
            "n_particles": 59,
            "friction_trans": ureg.Quantity(4.388999692570726e-07 ,"newton * second / meter"),
            "friction_rot": ureg.Quantity(6.902407326480637e-16 ,"meter * newton * second"),
            "rod_fixed": True,
        }
        maze_params ={
            "maze_particle_type": 2,
            "maze_dic": {'x_col_pos': [510.0, 510.0, 510.0, 550.0, 570.0, 550.0, 550.0, 570.0,
                                       510.0, 570.0, 570.0, 530.0, 530.0, 530.0, 550.0],
                         'y_col_pos': [530.0, 550.0, 570.0, 510.0, 510.0, 570.0, 550.0, 550.0,
                                       510.0, 530.0, 570.0, 510.0, 530.0, 570.0, 530.0], 
                         'destination': [530.0, 550.0], 
                         'offset': [500, 500], 
                         'wall_thickness': 2},
            "maze_walls":  [[500.0, 500.0, 500.0, 520.0],
        ...
        if mode in ["minimum","rod"]:
            logger.info("Add " + str(run_params["n_colloids"]) + " colloids to simulation")
            coll_type = 0
        ...
        if mode == "multiple_types":
            logger.info("Add " + str(run_params["n_colloids"]) + " colloids type 0 and also 1 to simulation")
        ...
        if mode == "maze":
            ...
            for col_i in range(len(maze_dic["x_col_pos"])):
                system_runner.add_colloid_precisely(
                        ...
                        init_position=ureg.Quantity([maze_dic["x_col_pos"][col_i],maze_dic["y_col_pos"][col_i],0],"micrometer"),
                        init_2D_angle=2*np.pi*np.random.rand(),
                        type_colloid=coll_type,
                )
            system_runner.add_maze(maze_params["maze_walls"], maze_params["maze_particle_type"],maze_dic["wall_thickness"])
        ...
        try:
            ...
            logger.info("I can't close the data file for the written_info in the visualization."
        ...
        if "written_info_data.txt" in files:
            ...
            #written_info_data = data[:-run_params["write_interval_fac"]:run_params["write_interval_fac"]]
            ...
            with open(outfolder + "/vision_cone_data.pick", 'rb') as f:
                ...
                while True:
                    try:
                        ...
                        break
            #vision_cone_data = data[:-run_params["write_interval_fac"]:run_params["write_interval_fac"]]
        ...
        logger.info("Writing parameters and additional data"
    ...
    def test_minimum(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if outfolder is not None:
                ...
                raise Exception("You need to specify where your extradata for visualization is located."
                 "It is assumed that it lies where the trajectory.hdf5 file is located.")
    ...
    def test_maze(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            ani_instance.animation_maze_setup(
                maze_folder, maze_file_name, parameters["maze_params"]["maze_dic"], parameters["maze_params"]["maze_walls"]
            )
            ani_instance.ax.plot(parameters["maze_params"]["maze_dic"]["destination"][0],parameters["maze_params"]["maze_dic"]["destination"][1],'xr')
