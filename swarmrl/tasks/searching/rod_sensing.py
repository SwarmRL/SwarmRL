...
class RodSensing(Task, ABC):
    """
    Find the rod-area in a box using distances.
    """

    def __init__(
        self,
        rod_center: np.array = np.array([0.0, 0.0, 0.0]),
        rod_length: float = 100,
        ...
        """
        Constructor for the find rod-area task.

        Parameters
        ----------
        rod_center : pint.Quantity (default = (0, 0, 0))
                Source of the rod.
        rod_length : pint.Quantity (default = 100)
                Length of the rod
        decay_function : callable (required=True)
                A function that describes the decay of the field along one dimension, for points further away, than the rod-area radius
        ...
        self.source = rod_center / box_length
