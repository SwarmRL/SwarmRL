...
class DrugDelivery(Task):
    ...
    def __init__(
        ...
        self.old_drug_pos = None
        self.num_colloids = None
    ...
    def initialize(self, colloids: List[Colloid]):
        ...
        self.old_drug_pos = colloids[self.drug_index].pos / self.box_length

    def _compute_reward1(self, old_drug_dest_dist, new_drug_dest_dist):
        delta_dist = old_drug_dest_dist - new_drug_dest_dist
        reward = np.where(delta_dist < 0, 0, delta_dist)
        return 2000 * reward

    def _compute_reward2(self, drug_dest_pos, a=213, b=190, c=-0.06):
        return 10 / (1 + a * np.exp(b * (drug_dest_pos + c)))
    ...
    def __call__(self, colloids: List[Colloid]):
        ...
        new_drug_position = colloids[self.drug_index].pos / self.box_length

        # compute the distance between the transporter and the drug
        old_drug_dest_dist = np.linalg.norm(self.old_drug_pos - self.destination)

        new_drug_dest_dist = np.linalg.norm(new_drug_position - self.destination)

        self.old_drug_pos = new_drug_position

        reward1 = self._compute_reward1(old_drug_dest_dist, new_drug_dest_dist)
        reward2 = self._compute_reward2(new_drug_dest_dist)

        final_reward = reward1 + reward2
        return final_reward * np.ones(self.num_colloids)
